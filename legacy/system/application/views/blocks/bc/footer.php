<footer id="bee_footer">
    <div class="container">
        <div class="row">
            <div class="col-md-3 col-sm-3 col-xs-6">
                <div class="footer_menu_items">
                    <a class="" href="<?php echo SURL . 'stores/storelist/' ?>">ALL STORES</a>
                </div>
            </div>

            <div class="col-md-3 col-sm-3 col-xs-6">
                <div class="footer_menu_items">
                    <a class="" href="<?php echo SURL . 'info/terms/' ?>">TERMS OF SERVICE</a>
                </div>
            </div>

            <div class="col-md-3 col-sm-3 col-xs-6">
                <div class="footer_menu_items">
                    <a class="" href="<?php echo SURL . 'info/privacy/' ?>">PRIVACY POLICY</a>
                </div>
            </div>
            <div class="col-md-3 col-sm-3 col-xs-6">
                <div class="footer_menu_items">
                    <a class="" href="<?php echo SURL . 'info/contact/' ?>">CONTACT US</a>
                </div>
            </div>
        </div>
        <div class="row" id="footer_border"></div>
        <div class="row">
            <div class="col-md-3">
                <div class="footer_logo">
                    <img class="img-responsive" src="<?php echo ASSETURL ?>/images/logo.png">
                </div>
            </div>
            
            
            
            <div class="col-md-9">
                <div class="bee_footer_content">
                    <p>Tax and shipping costs are estimates;  please see the store's website for exact pricing.  BeeSavy does not guarantee the accuracy of information provided by online stores and other third parties, including product information, prices, coupons, and availability.  BeeSavy shall not be liable for or responsible to honor any inaccurate information shown on our website.  Please see our Terms of Service for more details.</p>
                </div>
            </div>
        </div>
    </div>
</footer>




<section id="bee_copy_rights">
    <div class="container">
        <div class="copy_rights">
            <span>Copyright &copy; 2011 - 2017 BeeSavy, LLC.  All Rights Reserved. </span>
        </div>
    </div>
</section>

