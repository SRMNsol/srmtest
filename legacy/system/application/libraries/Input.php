...
/**
 * This input class is a modified version from the original CI_Input
 *
 * This will allow remote IP to be set from HTTP_X_FORWARDED_FOR without
 * crosschecking REMOTE_ADDRESS with configuration proxy_ips list
 *
 * This is necessary when application is behind a proxy or load balancer
 * with dynamic IP such as Amazon ELB, to allow session data check by IP
 * to work properly
 *
 * Ideally HTTP access to web server instances should only be coming
 * from the ELB to trust HTTP_X_FORWARDED_FOR data
 */
class CI_Input {
	...
	*/
	function ip_address()
	{
        ...
        if (config_item('proxy_ips') != '') {
            throw new Exception('Proxy ips is not used in this customized input class');
        }

        /**
         * Modified part, ELB will set HTTP_X_FORWARDED_FOR
         */
