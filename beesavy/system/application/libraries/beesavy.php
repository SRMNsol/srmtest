...
class Beesavy extends Extrabux {
    ...
    function get_hot_coupons($num_coupons){
        $coupons = Extrabux::listCoupon('','','',50);
        $ids = array();
        $hot = array();
    ...
    function getUserStats($id){
        $cashback = Extrabux::getUserStats($id);
    ...
    function listCoupon($merchant_id, $page, $sort, $limit){
        $coupons = Extrabux::listCoupon($merchant_id, $page, $sort, $limit);
        foreach($coupons['coupons'] as &$coupon){
    ...
    function listStore($q, $page, $sort, $limit, $category){
        $stores = Extrabux::listStore($q, $page, $sort, $limit, $category);
    ...
    function getStore($id){
        $store = Extrabux::getStore($id);
        $store['link'] = '/transfer/store/'.$id;
        return $store;
    }
    function compareprices($groupID, $zip = False){
        $products = Extrabux::compareprices($groupID, $zip);
        function comp($a, $b){
            $valA = ($a['final_amount']);
